...
describe("Multiplier3 with PLONK", function () {
    ...
    it("Should return true for correct proof", async function () {
        ...
        let [m,n]=calldata.split(',');
        n =n.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());
        expect(await verifier.verifyProof(m,n)).to.be.true;
    });
    it("Should return false for invalid proof", async function () {
        //[assignment] insert your script here
        let wrong_a=0;
        let wrong_b=[0];
        expect(await verifier.verifyProof(wrong_a,wrong_b)).to.be.false;
    });
});